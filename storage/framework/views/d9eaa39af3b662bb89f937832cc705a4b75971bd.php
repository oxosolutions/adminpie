...
<div id="add_media" class="modal modal-fixed-footer aione-media-modal" style="overflow-y: hidden;">
	...
	<div class="modal-footer">
		...
		<?php echo Form::open(array('url' => 'foo/bar', 'class' => 'form_media_upload', 'files' => true, 'class')); ?>
        ...
        $('#field_3168 select').change(function(){
            ...
            $.ajax({
               ...
               success: function(result){
                    ...
                    $('#field_3169 select').val('<?php echo e(@$model['select_column']); ?>');
               }
            });
        });
...
window.Laravel = <?php echo json_encode([
	...
	$(document).ready(function(){
		...
		<?php if(in_array($field->field_type,['checkbox','radio','select'])): ?>
			$('.options').parents('.repeater-group').parent().show();
		<?php endif; ?>
			...
			$.ajax({
				...
				success : function(res){
					...
					if(res.extension == 'jpg' || res.extension == 'jpeg'){
						$('.gallery-form').find('.gallery-image img').attr('src',"<?php echo e(asset('media/')); ?>/"+res.original_name);
					}else if(res.extension == 'mp3'){
						$('.gallery-form').find('.gallery-image audio > source').attr('src',"<?php echo e(asset('media/')); ?>/"+res.original_name);
					}else if(res.extension == 'mp4'){
						$('.gallery-form').find('.gallery-image video > source').attr('src',"<?php echo e(asset('media/')); ?>/"+res.original_name);
					}
