...
use Hash;
use DB;
class EmployeeController_old extends Controller
{
    protected $user;
               ...
               try{
                    ...
                    DB::beginTransaction();
                        ...
                        if($alreadyExists == null){
                            ...
                            $user->save();
                            $uid = $user->id;
                            if(isset($value['employee_id'] && !empty($value['employee_id']))){
                                $this->add_user_meta($uid, 'employee_id' ,$value['employee_id'] );
                            }

                            // $userMeta = new UsersMeta();
                            // $userMeta->user_id =  $uid;
                            // $userMeta->key     =  'employee_id';
                            // $userMeta->value   =  $value['employee_id'];
                            // $userMeta->save();

                            if(!empty($value['designation'])){
                                $des_check  = DES::select(['id'])->where(['name'=>$value['designation']]);
                                    if($des_check->exists()){
                                    ...
                                    $this->add_user_meta($uid, 'designation' , $des_id);
                                    
                                // $userMeta = new UsersMeta();
                                // $userMeta->user_id =  $uid;
                                // $userMeta->key     =  'designation';
                                // $userMeta->value   =  $des_id;
                                // $userMeta->save();
                            }

                            if(!empty($value['department'])){
                                $dep_check  = DEP::select(['id'])->where(['name'=>$value['department']]);
                                if($dep_check->exists()){
                                    $dep_id = $dep_check->first()->id;
                                }else{
                                    $dep = new DEP();
                                    $dep->name = $value['department'];
                                    $dep->status = 1;
                                    $dep->save();
                                    $dep_id = $dep->id;
                                }

                                $this->add_user_meta($uid, 'department' , $dep_id);

                                // $userMeta = new UsersMeta();
                                // $userMeta->user_id =  $uid;
                                // $userMeta->key     =  'department';
                                // $userMeta->value   =  $dep_id;
                                // $userMeta->save();
                            }

                            if(!empty($value['date_of_joining'])){

                                $this->add_user_meta($uid, 'date_of_joining' , date('Y-m-d', strtotime($value['date_of_joining'])));
                                // $userMeta = new UsersMeta();
                                // $userMeta->user_id =  $uid;
                                // $userMeta->key     =  'date_of_joining';
                                // $userMeta->value   =  date('Y-m-d', strtotime($value['date_of_joining']));
                                // $userMeta->save();
                            }
