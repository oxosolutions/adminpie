    ...
    <link href="{{ asset('css/ocrm.css') }}" rel="stylesheet"  media="screen,projection"/>
	<link rel="stylesheet" href="{{ asset('css/spectrum.css') }}">
	<link rel="stylesheet" href="{{ asset('css/simple-iconpicker.min.css') }}">
    ...
    <script  src="{{ asset('js/common.js?')}}<?php echo rand(4524,28282); ?>"></script>

	<script  src="https://www.appelsiini.net/download/jquery.jeditable.js"></script>
	<script  src="{{ asset('js/spectrum.js') }}"></script>
	<script  src="{{ asset('js/simple-iconpicker.min.js') }}"></script>
	    		...
	    		@foreach(@$plugin as $ikey => $file)
				    @if(is_array($file) && $ikey == 'custom')
						@foreach(@$file as $iKey => $iVal)
				        	<script  src="{{asset('js/pages/'.$iVal.'.js')}}?ref={{rand(8899,9999)}}"></script>
						@endforeach
				    @else
