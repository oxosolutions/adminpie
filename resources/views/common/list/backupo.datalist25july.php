		...
		<li class="aione-datalist-item aione-datalist-header-item" >
			@foreach($showColumns as $k => $column)
				@if($loop->index == 0)
					...
					<div class="column aione-column-{{(is_array($column))?strtolower(str_replace(' ', '-', $column['title'])):strtolower(str_replace(' ', '-', $column))}}"" style="width: {{100/count($showColumns)}}%"><span class="column-order" column-key="{{$k}}" style="cursor: pointer;">
						@if(is_array($column))
							{{$column['title']}}
						@else
							{{$column}}
				...
				@else
					...
					@endphp
					<div class="column aione-column-{{(is_array($column))?strtolower(str_replace(' ', '-', $column['title'])):strtolower(str_replace(' ', '-', $column))}}" style="width: {{100/count($showColumns)}}%">
