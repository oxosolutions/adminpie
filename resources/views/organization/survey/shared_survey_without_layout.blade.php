...
<div class="ar">
	<div class="ac l24">
				            	...
				            	@foreach($surveyVal['fields'] as $fields)
									...
									@if(!empty($current_data))
										@if(array_key_exists($slug , array_filter($current_data)))
											<li class="aione-nav-item level1 ui-sortable-handle fill_{{$slug}}">
							                    <a href="">
							                        <span class="nav-item-text">{{$fields['field_title']}} {{$slug}}</span>
							                    </a>
							                </li>
										@else
											<li class="aione-nav-item level1 ui-sortable-handle fill_{{$fields['field_slug']}}">
							                    <a href="">
							                        
							                        <span class="nav-item-text">{{$fields['field_title']}} {{$fields['field_slug']}}</span>
							                    </a>
							                </li>
										@endif
									@else
						                <li class="aione-nav-item level1 ui-sortable-handle fill_{{$fields['field_slug']}}">
						                    <a href="http://admin.scolm.com/modules/76/355">
						                        ...
						                        <span class="nav-item-text">{{substr($fields['field_title'], 0,40)}} </span>
						                        <span style="margin-left: -18px" class="pl-18 display-block font-size-12 ans_{{$fields['field_slug']}}"> 
		...
		{{-- <div class="aione-border p-10">
			<ul class="aione-collapsible" data-collapsible="accordion">
				@foreach($survey['section'] as $surveyVal)
					<div class="aione-item">
						...
						@if(!empty($surveyVal['fields']))
					      	@foreach($surveyVal['fields'] as $fields)
								...
								@if(!empty($current_data))
									@if(array_key_exists($slug , array_filter($current_data)))
										<div class="aione-item-content bg-red">
											{{$fields['field_slug']}}
										</div>
										@else
