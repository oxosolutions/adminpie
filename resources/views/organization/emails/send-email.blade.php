...
@include('common.page_content_primary_start')
    {!!Form::model(@$model,['route'=>'save.campaign','id'=>'campaign_form'])!!}
        <div class="row">
        	<div class="col l9 pr-7">
    			...
    			<div class="aione-field-wrapper">
    				<div class="label">
    				 ...
    				 {!!Form::text('campaign_name',null,['class'=>'no-margin-bottom aione-field','placeholder'=>'Subject'])!!}
                     @if($errors->has('campaign_name'))
                        <span style="color: red; font-size: 12px;">{{$errors->first('campaign_name')}}</span>
                     @endif
    			</div>
    			<div class="aione-field-wrapper">
    				<div class="label">
    				 ...
    				 {!!Form::textarea('campaign_desc',null,['size' => '100x100','class'=>'no-margin-bottom aione-field','placeholder'=>'Message','style'=>'height:300px'])!!}
                     @if($errors->has('campaign_desc'))
                        <span style="color: red; font-size: 12px;">{{$errors->first('campaign_desc')}}</span>
                     @endif
    			</div>
    			<div>
        	...
        	<div class="col l3 pl-7">
        		<div class="card">
        			...
        			<div class="content" >
                        ...
                        @endphp
                        {!!Form::select('users[designation][]',$designations,@$model->selected_users['designation'],['class'=>'designation '.$boxClass,'multiple'=>'multiple'])!!}
                        @php
                            if(@$model != null){
                        ...
                        @endphp
                        {!!Form::select('users[department][]',$departments, @$model->selected_users['department'], ['class'=>'department '.$boxClass,'multiple'=>'multiple'])!!}
                        ...
                        {!!Form::select('users[shifts][]',$departments,@$model->selected_users['shifts'],['class'=>'shifts '.$boxClass,'multiple'=>'multiple'])!!}
                        @php
                            if(@$model != null){
                        ...
                        @endphp
                        {!!Form::select('users[roles][]',$roles,@$model->selected_users['roles'],['class'=>'roles '.$boxClass,'multiple'=>'multiple'])!!}
                        @php
                            if(@$model != null){
                        ...
                        @endphp
                        {!!Form::select('users[users][]',$users,@$model->selected_users['users'],['class'=>'users '.$boxClass,'multiple'=>'multiple'])!!}
        			</div>
        			<div class="row">
        		...
        		<div class="card">
        			...
        			<div class="content">
                        {!!Form::select('layout',$layouts,null,['class'=>'browser-default','placeholder'=>'Select Layout'])!!}
        				
                        @if($errors->has('layout'))
                            <span style="color: red; font-size: 12px;">{{$errors->first('layout')}}</span>
                        @endif
        			</div>
        		</div>
        		<div class="card">
        			...
        			<div class="content">
                        {!!Form::select('template',$templates,null,['class'=>'browser-default','placeholder'=>'Select Template'])!!}
                        @if($errors->has('template'))
                            <span style="color: red; font-size: 12px;">{{$errors->first('template')}}</span>
                        @endif
        			</div>
        		</div>
        	</div>  
        ...
        @include('common.page_content_secondry_start')
        	...
        	@include('common.modal-onclick',['data'=>['modal_id'=>'schedule_later','heading'=>'When you want to send this email','button_title'=>'Send','section'=>'emasec4']])
        	{{-- add email modal end--}}
