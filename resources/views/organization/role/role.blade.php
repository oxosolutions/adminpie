...
{{-- @php
  if(isset($_GET['id'])){
    $current_role = $_GET['id'];
    $check = App\Model\Organization\OrganizationSetting::getData($current_role);
    if(count($check) > 0){
      dump($check);
    }else{
      dump('empty');
                  ...
                  <div class="col s8 m8 l8">
                    ...
                    <a href="" data-toggle="popover" title="" data-content="TEST" >
                      <h5 class="project-title black-text flow-text truncate line-height-35">
                        <span class="project-name shift_name font-size-14">{{$value->name}}</span>
                      </h5>
                    </a>
                  </div>
                  {{-- <a href="{{Route('role.delete',['id'=>$value->id])}}"><i class="fa fa-trash red-text"></i></a> --}}
                  <div class="col s4 m4 l4 options">
                    <a href="{{Route('role.assign',['id'=>$value->id])}}">Assign Permission</a>
                    <a href="{{Route('role.delete',['id'=>$value->id])}}" id="delete_role" role-id="{{$value->id}}"><i class="fa fa-trash red-text"></i></a>
                    <div id="change_role" class="modal ">
                      ...
                      <div class="modal-content">
                       ...
                       {!! Form::open(['metod' => 'post' , 'route' => 'role.delete']) !!}
                          <select name="change_role">
                            <option disabled="disabled">Select new Role to Replace With:</option>
                              @foreach(App\Model\Organization\UsersRole::getRoles() as $key => $list)
